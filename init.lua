...
do
  ...
  -- Ensure the undo directory exists before setting undodir
  ...
  vim.filetype.add {
    extension = {
      html = function(path, bufnr)
        -- Check if buffer is loaded before searching
        if vim.api.nvim_buf_is_loaded(bufnr) then
          -- Save current cursor position
          local cursor = vim.api.nvim_win_get_cursor(0)
          -- Search for Django template tags
          local found = vim.fn.search('{%\\|{#\\|{{', 'nw') ~= 0
          -- Restore cursor position
          pcall(vim.api.nvim_win_set_cursor, 0, cursor)
          if found then return 'htmldjango' end
        end
        return 'html'
      end,
    },
...
do
  ...
  vim.keymap.set('n', '<leader>wd', '<C-w><C-d>', { desc = 'Open Diagnostic Floating Window' })
...
do
  ...
  require('todo-comments').setup {
    ...
    keywords = {
      ...
      EXPLAIN = { icon = '󰂺', color = 'hint', alt = { 'EXPLANIATION', 'EXPLAINING', 'EXP', 'WHY' } },
    },
  }
...
do
  ...
  -- Multi-grep: separate pattern and file glob with two spaces
...
do
  ...
  --
  -- Note: LSP servers defined in the `servers` table above are automatically included.
  -- This list adds additional language servers and tools not in that table.
